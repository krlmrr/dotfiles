...
- [yabai: background native tabs hold invisible tiles](project_yabai_native_tab_windows.md) — absent from `query --windows` but resolvable individually; their own `.space` reports the FOCUSED space, so never trust it
- [yabai animations are impossible under SIP](project_yabai_sip_no_animation.md) — `window_animation_duration` is refused outright; stop looking for smoothing, reduce the number of moves instead
...
- [Laravel LSP in Zed via dev extension](project_laravel_lsp_zed.md) — pending PR #6996; when it merges, swap to the marketplace extension and remove the rustup provisioning in Brewfile + setup
- [yabai AX loss = phantom windows + restack-every-visit](project_yabai_ax_loss.md) — AX-dead windows report empty role/subrole, stay in the space list at old geometry, invisible to every predicate; off-screen geometry reads are pre-op (why WARP_SETTLE flip-flopped); two plausible fixes disproven
- [yabai --insert parity trap](project_yabai_insert_parity.md) — armed points survive warps; never prime, verify+repair instead
- [yabai: the browser owns the TOP-LEFT](project_yabai_browser_left.md) — spaces 3+, topmost of the leftmost column is a browser and EVERY browser window is in that column (repair `south`, 3 passes); `window_origin_display focused` beats Chrome's saved display; two weaker rules were tried and rejected (a rule's REPAIR is part of the rule — "strictly left" manufactures columns); yabairc's NO-ARG path is the full config load (don't invoke it incidentally — it duplicates every signal); split_type stays auto; don't test by moving live windows
- [yabai display topology + zero-motion limit](project_yabai_browser_incremental.md) — Studio Display is 3200×1800 (NOT the laptop; laptop is 2056×1329); window_created fires post-BSP so one correction hop is unavoidable. The trio/slot architecture this file documented is SUPERSEDED
