...
- [yabai: one invariant — the browser owns the left](project_yabai_browser_left.md) — spaces 3+, leftmost browser strictly left of every non-browser; yabairc's NO-ARG path is the full config load (don't invoke it incidentally — it duplicates every signal); split_type must stay auto; don't test by moving live windows
