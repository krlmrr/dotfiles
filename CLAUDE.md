...
- `shared/` — cross-platform configs (nvim, ghostty, lazygit, zed, zsh, git, jj, fonts, wallpapers)
- `mac/` — macOS-specific (hammerspoon, vscode, zed keymap, zsh aliases)
...
| --- | --- | --- |
| **Symlinked** | Live — change is immediate | nvim, lazygit, zed, ghostty `themes/`, claude skills, `gitignore_global`, jj `config.toml`, all `mac/*` configs (hammerspoon, vscode, sketchybar, yabai), arch `hypr/waybar/rofi/mako`, keyd |
...
| **Copied (one-shot)** | Re-run `./configure` (or the step that copies it) | `.gitconfig` (copied to avoid `git config --global` polluting the repo), COSMIC `~/.config/cosmic/*` (COSMIC rewrites its own files; `cp -a --force` from `linux/shared/cosmic/`), fonts, Zen `user.js` (Zen ignores symlinks), `~/.config/jj/conf.d/00-identity.toml` (generated by `configure` from the git identity; no repo source, never committed) |
