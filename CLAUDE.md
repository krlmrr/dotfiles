...
| --- | --- | --- |
| **Symlinked** | Live — change is immediate | ghostty config + `themes/` (both `~/.config/ghostty` and the macOS app-support location), `.gitconfig`, `gitignore_global`, nvim, lazygit, zed (`settings.json`, `themes/`, `keymap.json`), jj `config.toml`, herdr `config.toml`, claude skills, hammerspoon, vscode, sketchybar, yabai |
