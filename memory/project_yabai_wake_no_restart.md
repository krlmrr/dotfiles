...
(Historical, and the editor role is gone as of 2026-08-25 — see [[project-yabai-browser-left]]: placement was made deterministic and idempotent, skipping when the layout was already correct. The idempotent-skip property survives in today's invariant check.) (Historical: a `^ghostty$ manage=off` rule was believed to exclude a "phantom" lowercase `ghostty` window — **REMOVED 2026-07-15 as a proven no-op.** The lowercase roleless `ghostty` is NOT a separate phantom: it's yabai's **de-hooked view of the REAL Ghostty terminal** (`has-ax-reference=false`, `role=""`) after a yabai restart while Ghostty was already open. yabai never tiles roleless windows regardless of any rule, and the healthy window is `Ghostty` (capital), which the case-sensitive `^ghostty$` never matched — so the rule did nothing in every case. Remedy for the de-hooked state (Ghostty won't tile, floats, Zen fills the space): `yabai --restart-service` re-acquires the AX handle. See [[project-yabai-browser-incremental]].)
