...
**Consequences while this is unresolved:** the dev extension never auto-updates, so this machine is frozen at `23f7e06` and needs a manual `git pull` + rebuild in `~/Code/laravel-zed-extension` for LSP updates — and the rustup provisioning in `Brewfile` + `setup` must **stay** (don't remove it yet).
...
2. Remove the rustup provisioning — `brew "rustup"` in `Brewfile`, and the unlink/force-link/`rustup default stable` block in `setup` — since marketplace extensions ship precompiled wasm and need no Rust toolchain. Keep it only if rustup is wanted for other reasons. **Note:** this was "parked and uncommitted" as of 2026-07-28, so the old instruction was `git checkout mac/Brewfile mac/setup.sh`. v2.0.0 flattened those paths to `Brewfile` + `setup` AND committed the provisioning, so a `git checkout` no longer reverts anything — the lines must be deleted.
